...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['name'], $_POST['description'], $_POST['price'], $_POST['category'], $_POST['cousin_type'], $_FILES['image'])) {
        // Get form data
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
                    $image_path = $uniqueFilename; // Store just the filename
                } else {
                    $errors[] = "Sorry, there was an error uploading your file.";
                }
            }
        } else {
            $errors[] = "No image file was uploaded or there was an upload error.";
        }

        // Insert data into database if there are no errors
        if (empty($errors) && !empty($image_path)) {
            $query = "INSERT INTO menu (name, description, price, category, cousin_type, image) VALUES ('$name', '$description', '$price', '$category', '$cousin_type', '$image_path')";

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cousin_type = mysqli_real_escape_string($con, $_POST['cousin_type']);
    ...
    if (empty($errors)) {
        if (!empty($image_path)) {
            ...
            $update_query = "UPDATE menu SET name = ?, description = ?, price = ?, category = ?, cousin_type = ?, image = ? WHERE id = ?";
            $stmt = mysqli_prepare($con, $update_query);
            mysqli_stmt_bind_param($stmt, "ssdsssi", $name, $description, $price, $category, $cousin_type, $image_path, $food_id);
        } else {
            // Update without changing image path
            $update_query = "UPDATE menu SET name = ?, description = ?, price = ?, category = ?, cousin_type = ? WHERE id = ?";
            $stmt = mysqli_prepare($con, $update_query);
            mysqli_stmt_bind_param($stmt, "ssdssi", $name, $description, $price, $category, $cousin_type, $food_id);
        }
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?id=" . $food_data['id']; ?>" method="post"
            ...
            <select id="category" name="category" required>
                ...
                <option value="Main Dishes"
                    <?php if ($food_data['category'] == 'Main Dishes') echo 'selected'; ?>>Main Dishes</option>
            ...
            <select id="cousin_type" name="cousin_type" required>
                <option value="">Select a cuisine type...</option>
                <option value="Sri Lankan"
                    <?php if ($food_data['cousin_type'] == 'Sri Lankan') echo 'selected'; ?>>Sri Lankan</option>
                <option value="Chinese"
                    <?php if ($food_data['cousin_type'] == 'Chinese') echo 'selected'; ?>>Chinese</option>
                <option value="Italian"
                    <?php if ($food_data['cousin_type'] == 'Italian') echo 'selected'; ?>>Italian</option>
            </select>
